...
import VsSearchBar from "./VsSearchBar";
import VsUserCard from "./VsUserCard";
import VsMetricsTable from "./VsMetricsTable";
import VsHead2Head from "./VsHead2Head";
import VsRadarChart from "./VsRadarChart";
import LoadingSpinner from "../common/LoadingSpinner";
import useVsComparison from "../../hooks/useVsComparison";
const VsContainer = ({ onBack }) => {
  const { t } = useLanguage();
  const {
    user1,
    user2,
  ...
  return (
    <div className="min-h-screen transition-all duration-700 bg-md-background">
      {" "}
      <div className="container mx-auto px-4 sm:px-6 lg:px-8 py-8 max-w-7xl">
        ...
        <div className="mb-6 animate-slide-up">
          ...
          >
            ...
            <span>{t("vsBack") || "Back to Home"}</span>{" "}
          </button>{" "}
        </div>{" "}
        <div
        ...
        >
          {" "}
          <h1 className="text-4xl sm:text-5xl lg:text-6xl font-bold font-display text-md-on-surface">
            {" "}
            <Icon icon={I.swordCross} className="inline-block mr-3" /> VS Mode{" "}
          </h1>{" "}
          <p className="mt-3 text-lg text-md-on-surface-variant">
            {" "}
            {t("vsSubtitle") || "Compare two GitHub profiles head-to-head"}{" "}
          </p>{" "}
        </div>{" "}
        <div
        ...
        {error && (
          <div className="mb-6 p-5 rounded-[24px] bg-md-error/10 border border-md-error/30 animate-shake animate-fade-in">
            {" "}
        ...
        {!comparisonData && !loading && !error && (
          <div className="text-center py-16 animate-fade-in">
            {" "}
            <div className="text-7xl sm:text-8xl mb-6 animate-float">
              <Icon icon={I.swordCross} size="3em" />
            </div>{" "}
            ...
            <p className="text-lg max-w-md mx-auto text-md-on-surface-variant">
              {" "}
              {t("vsEmptyDesc") ||
                "Enter two GitHub usernames above to compare their profiles and see who wins!"}{" "}
            </p>{" "}
          </div>
        )}{" "}
      </div>{" "}
